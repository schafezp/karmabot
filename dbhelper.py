...
def save_or_create_user_in_chat(user: User, chat_id: int, conn, change_karma=0) -> User_in_chat:
    with conn:
        with conn.cursor() as crs: #I would love type hints here but psycopg2.cursor isn't a defined class
            selectcmd = "SELECT id, user_id, chat_id, karma FROM user_in_chat uic where uic.user_id=%s AND uic.chat_id=%s"
            ...
            if result is None:
                ...
                karma = result[3] 
            if change_karma is not None:
                karma = karma + change_karma
            #TODO: set assumes only karma will change
            if result is None:
                #don't supply id to generate from SERIAL
                insertcmd = """INSERT into user_in_chat 
                (user_id, chat_id, karma) VALUES (%s,%s,%s)
                ON CONFLICT (id) DO UPDATE 
                SET karma = EXCLUDED.karma
                RETURNING id, karma """
                crs.execute(insertcmd,[user.get_user_id(),chat_id, karma])
            else:
                insertcmd = """INSERT into user_in_chat 
                (id,user_id, chat_id, karma) VALUES (%s,%s,%s,%s)
                ON CONFLICT (id) DO UPDATE 
                SET karma = EXCLUDED.karma
                RETURNING id, karma """
                crs.execute(insertcmd,[result[0],user.get_user_id(),chat_id, karma])
            ...
            id = row[0]
            karma = row[1]
            return User_in_chat(id,user.id,chat_id,karma)
...
def user_reply_to_message(user: User, chat: Telegram_chat , original_message : Telegram_message, reply_message : Telegram_message, karma : int, conn):
    ...
    (message_id,chat_id, author_user_in_chat_id, message_text) 
    VALUES (%s,%s,%s,%s)
    ON CONFLICT (message_id) DO UPDATE
    SET message_text = EXCLUDED.message_text;
    """
    selecturtm = """SELECT * from user_reacted_to_message urtm where urtm.user_in_chat_id=%s and urtm.message_id=%s and urtm.react_message_id=%s"""
    inserturtm = """INSERT INTO user_reacted_to_message 
    (user_in_chat_id,message_id,react_score,react_message_id)
    VALUES (%s,%s,%s,%s)"""
    with conn:
        with conn.cursor() as crs:
            ...
            if crs.fetchone() is None:
                argsurtm = [uic.id, original_message.message_id, karma, reply_message.message_id]
                crs.execute(inserturtm,argsurtm)
            
def get_karma_for_users_in_chat(chat_id: int,conn):
    cmd = """select username, first_name, last_name, karma from user_in_chat uic 
    LEFT JOIN telegram_user tm on tm.user_id=uic.user_id
    where uic.chat_id=%s"""
    with conn:
        with conn.cursor() as crs:
