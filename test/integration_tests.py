...
from karmabot.responses import (
    START_BOT_RESPONSE,
    SUCCESSFUL_CLEAR_CHAT,
    SHOW_KARMA_NO_HISTORY_RESPONSE,
)
from karmabot.commands_strings import (
    START_COMMAND,
    CLEAR_CHAT_COMMAND,
    SHOW_KARMA_COMMAND,
    USER_INFO_COMMAND,
...
class IntegrationTests(Common_Integration):
    ...
    def test_showkarma_works_on_empty_chat(self):
        ...
        clear_chat_response = self.client.send_command_await(
            CLEAR_CHAT_COMMAND, num_expected=1
        )
        ...
        show_karma_response = self.client.send_command_await(
            SHOW_KARMA_COMMAND, num_expected=1
        )
        self.assertEqual(len(show_karma_response.messages), 1)
        self.assertEqual(
            show_karma_response.messages[0].text, SHOW_KARMA_NO_HISTORY_RESPONSE
        )
    ...
    def test_votes_can_be_overriden(self):
        ...
        does_bot_have_1_karma = bool(
            re.search(f"{bot_name_without_at}: 1", bot_response)
        )
        ...
        does_bot_have_zero_karma = bool(
            re.search(f"{bot_name_without_at}: 0", show_karma_response.messages[0].text)
        )
        self.assertTrue(
            does_bot_have_zero_karma, "-1 on same message should override last vote"
        )
    ...
    def test_check_chat_karmas(self):
        response = self.client.send_command_await(
            SHOW_KARMA_KEYBOARD_COMMAND, num_expected=1
        )
